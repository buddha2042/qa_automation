...
interface Props {
  regularData: unknown;
  refactorData: unknown;
  comparisonReport: ComparisonItem[];
}

export default function AuditResultsView({ regularData, refactorData, comparisonReport }: Props) {
    ...
    <div className="min-h-screen bg-slate-50 pb-20">
      ...
      <main className="max-w-7xl mx-auto p-8">
        <div className="mb-10 bg-white border rounded-xl p-6 shadow-sm">
          ...
          <p className="text-sm text-slate-500">{mismatchCount} mismatches found in {comparisonReport.length} properties</p>

          <div className="mt-4">
            {mismatchCount === 0 ? (
              <span className="bg-green-100 text-green-700 px-4 py-1 rounded-full text-xs font-black">PASS</span>
            ) : (
              <span className="bg-red-100 text-red-700 px-4 py-1 rounded-full text-xs font-black">FAIL</span>
            )}
          </div>
        </div>
        ...
        <div className="bg-white border rounded-xl shadow overflow-hidden mb-12">
          <div className="px-6 py-4 border-b flex justify-between items-center">
            ...
            <button onClick={exportToExcel} className="text-xs font-bold border px-4 py-2 rounded hover:bg-slate-50">
              Download CSV
            </button>
          </div>
              ...
              <tbody>
                {comparisonReport.map((r, i) => (
                  <tr key={i} className={!r.isMatch ? 'bg-red-50/30' : ''}>
                    <td className="p-3 font-black text-[10px]">{r.isMatch ? 'MATCH' : 'DIFF'}</td>
